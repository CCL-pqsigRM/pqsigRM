...
void export_sk(unsigned char *sk, matrix *Sinv, uint16_t *Q, uint16_t *Qp, uint16_t *s_lead){
	...
	memcpy		(sk+Sinv->alloc_size+sizeof(uint16_t)*CODE_N, 
								   Qp, sizeof(uint16_t)*CODE_N/8);
	memcpy		(sk+Sinv->alloc_size+sizeof(uint16_t)*CODE_N+sizeof(uint16_t)*CODE_N/8, 
...
crypto_sign_keypair(unsigned char *pk, unsigned char *sk){
	...
	uint16_t *Qp = (uint16_t*)malloc(sizeof(uint16_t)*CODE_N/8);
